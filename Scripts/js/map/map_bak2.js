...
function dongArea(area, map, sido, sigungu) {
    ...
    kakao.maps.event.addListener(polygon, 'mouseover', function (mouseEvent) {
        var content = "<div class='area' align='center'>" + (sido + " " + sigungu + " " + area.name) + "</div>";
...
function sigunguArea(area, map, sido) {
    // 다각형을 생성합니다
    var polygon = new kakao.maps.Polygon({
        map: map, // 다각형을 표시할 지도 객체
        path: area.path,
    ...
    // 지역명을 표시하는 커스텀오버레이를 지도위에 표시합니다
    kakao.maps.event.addListener(polygon, 'mouseover', function (mouseEvent) {
        var content = "<div class='area' align='center'>" + (sido + " " + area.name) + "</div>";
    ...
    kakao.maps.event.addListener(polygon, 'click', function (mouseEvent) {
        ...
        MapClick(sido, area.name);

        $.ajax({
            type: "POST",
            url: "WebService.asmx/GetDongList",
            data: "{sido : '" + area.sido + "',sigungu : '" + area.name + "'}",
...
function sidoArea(area, map) {
    ...
    // 지역명을 표시하는 커스텀오버레이를 지도위에 표시합니다
    kakao.maps.event.addListener(polygon, 'mouseover', function (mouseEvent) {
        var content = "<div class='area' align='center'>" + area.name + "</div>";
    ...
    kakao.maps.event.addListener(polygon, 'click', function (mouseEvent) {
        ...
        if (area.name == "세종특별자치시") {
            ...
            for (var i = 0, len = 세종특별자치시.length; i < len; i++) {
                sigunguArea(세종특별자치시[i], map, "세종특별자치시")
            }
        }
        if (area.name == "경기도") {
        ...
        if (area.name == "제주특별자치도") {
            ...
            for (var i = 0, len = 제주특별자치도.length; i < len; i++) {
                sigunguArea(제주특별자치도[i], map, "제주특별자치도")
            }
        }
